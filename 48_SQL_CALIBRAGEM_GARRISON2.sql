-- =============================================================
-- KingsWorld — CALIBRAGEM: conquista MÉDIA (garrison 1 -> 2)
-- Rodar no SQL Editor (King's World!) APÓS 47. create-or-replace.
-- Meta do dono: conquistar/segurar cidade exige sobreviventes >= 2x o tamanho
-- da cidade (antes 1x). Punição (teto 50% + saque 25%) = mantida (recomendado).
-- Único diff vs versão do 42: c_garrison 1 -> 2.
-- =============================================================
...
declare
  ...
  c_garrison constant numeric := 2;       -- CONQUISTA MÉDIA: sobreviventes >= 2x tamanho
  c_att_loss_cap constant numeric := 0.5; -- punição mantida
  c_def_loss_cap constant numeric := 0.5;
begin
  ...
  -- ALVO ABANDONADO: ocupa se vencer e tiver gente (>= 2x tamanho)
  if v_def_owner is null then
  ...
  -- ALVO COM DONO: perdas do defensor + saque + conquista por herói
  update public.world_player_imperial_states set
